...
public class SanBiaoLiuShuiAdapter extends BaseAdapter {

	private ArrayList<ShouYiBean> list;
	private Context mContext;

	public SanBiaoLiuShuiAdapter(Context context, ArrayList<ShouYiBean> list) {
		super();
		this.mContext = context;
		this.list = list;
	}

	public SanBiaoLiuShuiAdapter(Context context) {
		super();
		this.mContext = context;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if ("701".equals(shouYiBean.getTransType())){
			holder.tv_detail.setVisibility(View.VISIBLE);
			holder.tv_detail.setText("现金" + MathUtil.subDouble((shouYiBean.getAmount() - shouYiBean.getFee()), 2)
					+ "元  奖金" + MathUtil.subDouble(shouYiBean.getFee(), 2)+ "元");
		}
		else if("752".equals(shouYiBean.getTransType())){
			holder.tv_detail.setVisibility(View.VISIBLE);
