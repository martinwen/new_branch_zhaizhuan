...
public class SanBiaoAdapter extends BaseAdapter {
	...
	public SanBiaoAdapter(Context context, ArrayList<SanBiaoListBean> list) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = View.inflate(mContext, R.layout.items_sanbiao, null);
		...
		holder.tv_time_daoqi.setText("到期时间：" + sanBiaoListBean.getEndTimeStr());//到期时间

		if("10".equals(sanBiaoListBean.getStatus())){
			holder.iv_status.setImageResource(R.drawable.zaitou_shouyizhong);
			if(sanBiaoListBean.isProCanTransfer()){//true | false  true 允许转让 ，false不允许转让
			...
			if(sanBiaoListBean.getBookClearAmount() > 0){//回款金额 >0 代表有回款
		...
		if("20".equals(sanBiaoListBean.getStatus())){//10  持有中  20 转让中  30 已转让 40 已结清   50 承接结清
			holder.iv_status.setImageResource(R.drawable.zaitou_shouyizhong);
			holder.tv_zhuanrang.setVisibility(View.VISIBLE);
		...
		if("30".equals(sanBiaoListBean.getStatus())){//10  持有中  20 转让中  30 已转让 40 已结清   50 承接结清
			holder.iv_status.setImageResource(R.drawable.zaitou_yizhuanrang);
			holder.tv_zhuanrang.setVisibility(View.GONE);
			holder.tv_time_daoqi.setText("转让成功：" + sanBiaoListBean.getTransferSuccessTimeStr());//转让成功
		}
