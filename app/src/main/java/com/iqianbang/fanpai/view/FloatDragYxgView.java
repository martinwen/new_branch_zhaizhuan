...
/**
 * Created by wenguangjun on 2017/5/26.
 */

public class FloatDragYxgView {
    private Activity context; // 上下文
    private static ImageView mImageView; // 可拖动按钮
    ...
    public static boolean isAdd;//标记是否添加优先购悬浮窗
    ...
    public static ImageView addFloatDragView(Activity context , RelativeLayout mViewContainer,
        ...
        FloatDragYxgView floatDragView = new FloatDragYxgView(context);
        ImageView imageView = floatDragView.getFloatDragView(clickListener);
        mViewContainer.addView(imageView);
        //外界调用addFloatDragView方法后，添加优先购悬浮窗
        isAdd = true;
        return imageView;
    }

    public static void removeFloatDragView(RelativeLayout mViewContainer) {
        mViewContainer.removeView(mImageView);
        //外界调用addFloatDragView方法后，添加优先购悬浮窗
        isAdd = false;
    }

    // 初始化实例
    private FloatDragYxgView(Activity context) {
        setScreenHW(context);
    ...
    private ImageView getFloatDragView(View.OnClickListener clickListener) {
        if (mImageView != null) {
            ...
            mImageView.setImageResource(R.drawable.youxiangou);
    ...
    private void setFloatDragViewParams(View floatDragView) {
        ...
        if (0 != moveX || 0 != moveY) {// 移动后的位置
            ...
            RelativeLayout.LayoutParams lpFeedback = new RelativeLayout.LayoutParams(
                    RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
            lpFeedback.setMargins(0, 0, 20, 100);
            ...
            public boolean onTouch(final View v, MotionEvent event) {
                ...
                switch (action) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (bottom > mScreenHeight- LockPatternUtil.dip2px(context,60)) {
                            bottom = mScreenHeight- LockPatternUtil.dip2px(context,60);
                            LogUtils.i("bottom=="+bottom);
                            top = bottom - v.getHeight();
                        }
                        LogUtils.i("left=="+left+"---top=="+top+"---right=="+right+"---bottom=="+bottom);
    ...
    private void setScreenHW(Activity context) {
        if (mScreenHeight < 0) {
            ...
            LogUtils.i("mScreenWidth=="+mScreenWidth);
            LogUtils.i("mScreenHeight=="+mScreenHeight);
        }
    }
}
