...
import com.iqianbang.fanpai.adapter.zhanneixin.TouZiHuiKuanAdapter;
...
public class TouZiHuiKuanFragment extends BaseFragment implements View.OnClickListener {
	...
	private TouZiHuiKuanAdapter adapter;
	...
	protected View initView() {
		...
		// 5.设置adapter
		adapter=new TouZiHuiKuanAdapter(mActivity, list);
		...
		switch (v.getId()){
			case R.id.tv_sign:
				...
				for (int i = 0; i < list.size(); i++) {
					if (TouZiHuiKuanAdapter.getIsSelected().get(i)) {
			...
			case R.id.tv_all:
				...
				if(selectAll){
					...
					for (int i = 0; i < list.size(); i++) {// 遍历list的长度，全选
						TouZiHuiKuanAdapter.getIsSelected().put(i, true);
					}
				}else{
					...
					for (int i = 0; i < list.size(); i++) {// 遍历list的长度，全不选
						TouZiHuiKuanAdapter.getIsSelected().put(i, false);
					}
				}
						...
						if ("0".equals(code)) {
							...
							if (StringUtils.isBlank(datastr)) {
								...
								if (isSuccess) {// 验签成功
									...
									adapter.setsubClickListener(new TouZiHuiKuanAdapter.SubClickListener() {
										@Override
										public void OntopicClickListener() {
											for (int i = 0; i < TouZiHuiKuanAdapter.getIsSelected().size(); i++) {
												if (TouZiHuiKuanAdapter.getIsSelected().get(i)) {
