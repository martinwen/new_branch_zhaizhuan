...
import com.iqianbang.fanpai.adapter.zhanneixin.HuiYuanQuanYiAdapter;
...
public class HuiYuanQuanYiFragment extends BaseFragment implements View.OnClickListener {
	...
	protected View initView() {
		...
		// 5.设置adapter
		adapter=new HuiYuanQuanYiAdapter(mActivity, list);
		...
		switch (v.getId()){
			case R.id.tv_sign:
				...
				for (int i = 0; i < list.size(); i++) {
					if (HuiYuanQuanYiAdapter.getIsSelected().get(i)) {
			...
			case R.id.tv_all:
				...
				if(selectAll){
					...
					for (int i = 0; i < list.size(); i++) {// 遍历list的长度，全选
						HuiYuanQuanYiAdapter.getIsSelected().put(i, true);
					}
				}else{
					...
					for (int i = 0; i < list.size(); i++) {// 遍历list的长度，全不选
						HuiYuanQuanYiAdapter.getIsSelected().put(i, false);
					}
				}
						...
						if ("0".equals(code)) {
							...
							if (StringUtils.isBlank(datastr)) {
								...
								if (isSuccess) {// 验签成功
									...
									adapter.setsubClickListener(new HuiYuanQuanYiAdapter.SubClickListener() {
										@Override
										public void OntopicClickListener() {
											for (int i = 0; i < HuiYuanQuanYiAdapter.getIsSelected().size(); i++) {
												if (HuiYuanQuanYiAdapter.getIsSelected().get(i)) {
